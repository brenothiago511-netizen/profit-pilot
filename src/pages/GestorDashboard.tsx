...
import { Clock, Check, Zap, TrendingUp } from 'lucide-react';
...
export default function GestorDashboard() {
  ...
  const totalMonth = monthRecords.reduce((s, r) => s + r.daily_profit, 0);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-emerald-400" /> Este Mês
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold text-emerald-400">{fmt(totalMonth)}</p>
            <p className="text-xs text-muted-foreground mt-1">{monthRecords.length} registro(s)</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
              <Check className="w-4 h-4 text-primary" /> Total Registrado
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold">{fmt(totalAll)}</p>
            <p className="text-xs text-muted-foreground mt-1">{records.length} registro(s) no total</p>
          </CardContent>
        </Card>
      </div>
                ...
                <tbody>
                  {records.slice(0, 50).map(r => (
                    <tr key={r.id}>
                      ...
                      <td>
                        {r.shopify_status === 'confirmed' ? (
                          <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">
                            <Zap className="w-3 h-3 mr-1" /> A Receber
                          </Badge>
                        ) : r.shopify_status === 'received' ? (
                          <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">
                            <Zap className="w-3 h-3 mr-1" /> A Receber
                          </Badge>
                        ) : (
